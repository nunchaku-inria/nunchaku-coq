...
# coq_makefile -I src src/nunchaku_coq_main.ml src/Nunchaku.v 
#
...
MLFILES:=src/nunchaku_coq_main.ml
...
install-natdynlink:
	...
	for i in $(CMXSFILESINC); do \
	 install -m 0755 $$i "$(DSTROOT)"$(COQLIBINSTALL)/$(INSTALLDEFAULTROOT)/`basename $$i`; \
	done
...
install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	install -d "$(DSTROOT)"$(COQLIBINSTALL)/$(INSTALLDEFAULTROOT); \
	for i in $(CMIFILESINC) $(CMOFILESINC) $(NATIVEFILESINC) $(GLOBFILESINC) $(VFILESINC) $(VOFILESINC); do \
...
uninstall_me.sh: Makefile
	...
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/$(INSTALLDEFAULTROOT) && \\\nfor i in $(CMIFILESINC) $(CMOFILESINC) $(NATIVEFILESINC) $(GLOBFILESINC) $(VFILESINC) $(VOFILESINC); do rm -f "`basename "$$i"`"; done && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "$(INSTALLDEFAULTROOT)" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
