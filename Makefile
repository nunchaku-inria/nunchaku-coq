...
install:$(if $(HASNATDYNLINK_OR_EMPTY),install-natdynlink)
	cd "src/" && for i in $(CMAFILES0) $(CMIFILES0) $(CMOFILES0) $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Nunchaku/$$i`"; \
...
install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/Nunchaku/html
	for i in html/*; do \
